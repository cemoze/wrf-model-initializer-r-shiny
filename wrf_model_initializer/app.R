...
ui = navbarPage(inverse = TRUE, "WRF Model Initializer",
                 ...
                 tabPanel("Summary About the App",
                          fluidPage(h1("WRF (Weather Research and Forecasting) Model Initializer"),
                                    ...
                                    br(),
                                    p("Even though this app can run the model from the data download to the WRF model initializing; it is strongly recommended to use/execute the final prepared bash script in UNIX platform due to the background process problems of R Shiny platform!"),
                                    br(),
                              ...
                              fluidRow(column(6, align = "center",
                                              ...
                                              fluidRow(column(12, align = "center",
                                                              ...
                                                              fluidRow(column(6,
                                                                       ...
                                                                       column(6,
                                                                              wellPanel(style = "backgroud: white",
                                                                                        selectInput("hist_dom", "Choose the domains that the output is desired!",
                 ...
                 tabPanel("Bash Script",
                          sidebarLayout(
                              sidebarPanel(align = "center",
                                           ...
                                           p("This tab provides a bash script to run WRF model in UNIX platform. 
                                ...
                                this app can also be used for creating bash scripts instead of executing WRF model.",
                                           ...
                                           radioButtons("datadownload",label = "Download included?",inline = T, choices = c("Yes","No"), selected = "No"),
...
server = function(input, output,session) {
    ...
    observeEvent(input$download,{
        ...
        if (datatype == "FNL") {
            ...
            fnl_downloader(email = "ozenc@itu.edu.tr",
                           pass = "beocem13756677",
                           path = dirpath_data,
    ...
    bash2 = reactive({
        ...
        if (datadownload == "Yes") {
            ...
            if (datatype == "FNL") {
                ...
                # setwd(dirpath_data)
                # fnl_dirpath = paste0("cd ",dirpath_data)
        ...
        dirpath_wps = dirpath_wps()
        # dirpath_wps = "/media/cem/data/Build_WRF/WPS-4.1/"
        ...
        dirpath_wrf = dirpath_wrf()
        dirpath_wrf = paste0(dirpath_wrf,"/run")
        dirpath_wrf = "/media/cem/data/Build_WRF/WRF-4.1.5/run/"
        ...
        datatype = datatype()
        #Linkgrib ile veriyi linkleme WPS icine
        if (as.character(unlist(input$datadir)[1])!= 0 & as.character(unlist(input$datadir2)[1])!= 0){
            dirpath_data2 = dirpath_data2()
            datalink = paste("./link_grib.csh ", dirpath_data2,"/*", sep = "")
        } else if (as.character(unlist(input$datadir2)[1])!= 0) {
        ...
        for (i in 1:length(sekans_times_spinup_final)) {
            ...
            # timeres = 21600
            Sys.setenv(TZ='GMT')
            shouldbe_ung = seq(date_time_start_wps,date_time_end_wps, by = as.numeric(timeres))
            shouldbe_ung = paste0("FILE:",strftime(shouldbe_ung, "%Y-%m-%d_%H"))
            shouldbe_ung = shouldbe_ung[length(shouldbe_ung)]
            ungrib_kontrol = paste0("if test -f '",shouldbe_ung,"'; then echo 'Ungrib.exe completed!'; else echo 'Ungrib.exe is not completed!'; exit; fi")
        ...
        for (i in 1:length(sekans_times_spinup_final)) {
            ...
            timeres = timeres()
            Sys.setenv(TZ='GMT')
            shouldbe_met = seq(date_time_start_wps,date_time_end_wps, by = as.numeric(timeres))
            mg_grid = expand.grid(max_dom_g,shouldbe_met)
            mg_grid$Var2 = strftime(mg_grid$Var2, "%Y-%m-%d_%H:%M:%S")
            shouldbe_met = paste0("met_em.d0",str_c(mg_grid$Var1,mg_grid$Var2,sep = "."),".nc")
            shouldbe_met = shouldbe_met[length(shouldbe_met)]
        ...
        for (i in 1:length(sekans_times_spinup_final)) {
            ...
            hist_int = hist_int()
            Sys.setenv(TZ='GMT')
            shouldbe_wrf = seq(date_time_start_wps,date_time_end_wps, by = as.numeric(hist_int)*60)
            wrf_grid = expand.grid(max_dom_wrf_g, shouldbe_wrf)
            wrf_grid$Var2 = strftime(wrf_grid$Var2, "%Y-%m-%d_%H:%M:%S")
            shouldbe_wrf = paste0("wrfout_d0",str_c(wrf_grid$Var1,wrf_grid$Var2,sep = "_"))
            shouldbe_wrf = shouldbe_wrf[length(shouldbe_wrf)]
    ...
    observe({
        ...
        download_script2 = function(data) {
            downloadHandler(
                filename = function() {
                    paste("bash_sequence_wrf_",date_time_start_wps,"_",date_time_end_wps,".bash", sep = "")
                },
                content = function(file) {
                    write(data, file, sep = "")
                }
        ...
        output$download_bash2 = download_script(bash2)
    })
